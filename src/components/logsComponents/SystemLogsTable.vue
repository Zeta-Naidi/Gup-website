    ...
    >
                  ...
                  <v-list-item>
                    ...
                    >
                      <template v-slot:selection="{item}">
                        <v-chip
                          close
                          :color="!!item.username ? 'primary' : 'red'"
      ...
      <template v-slot:expanded-row="{ headers, item }">
        <td :colspan="headers.length -1 " style="border-color: #63adf2;border-style: solid;" class="pr-0 pl-1">
          <div style="word-wrap: break-word;" v-if="!!item.value && JSON.parse(item.value)['usernameDeleted']">
            {{ !!item.value && getTranslation('DELETED_USERNAME') + ': ' + JSON.parse(item.value)['usernameDeleted'] }}
          </div>
          <div style="word-wrap: break-word;" v-else>
            {{ !!item.value && getTranslation('USERNAME') + ': ' + JSON.parse(item.value)['username'] + " " + getTranslation('EMAIL') + ': ' + JSON.parse(item.value)['EMAIL'] + " " + getTranslation('ROLE') + ': ' + JSON.parse(item.value)['userRelationship']}}
          </div>
        </td>
      </template>
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        {title: 'userAgent', key: 'userAgent', sortable: false},
        {title: 'Type', key: 'type'},
      ...
      usernameItems: [],
      typeItems: [
        {title: "SUCCESS_LOGIN", key: "SUCCESS_LOGIN"},
        {title: "LOGOUT", key: "LOGOUT"},
        {title: "SESSION_EXPIRED", key: "SESSION_EXPIRED"},
        {title: "WRONG_LOGIN", key: "WRONG_LOGIN"},
        {title: "SUCCESS_LOGIN_MFA", key: "SUCCESS_LOGIN_MFA"},
        {title: "SUCCESS_REGISTER_MFA", key: "SUCCESS_REGISTER_MFA"},
        {title: "WRONG_LOGIN_MFA", key: "WRONG_LOGIN_MFA"},
        {title: "WRONG_REGISTER_MFA", key: "WRONG_REGISTER_MFA"},
        {title: "CREATE_USER", key: "CREATE_USER"},
        {title: "UPDATE_USER", key: "UPDATE_USER"},
        {title: "DELETE_USER", key: "DELETE_USER"},
        {title: "USER_BLOCKED", key: "USER_BLOCKED"},
        {title: "IP_ADDRESS_BLOCKED", key: "IP_ADDRESS_BLOCKED"},
        {title: "SUCCESS_RESET_PASSWORD", key: "SUCCESS_RESET_PASSWORD"},
        {title: "WRONG_RESET_PASSWORD", key: "WRONG_RESET_PASSWORD"},
      ],
      menuFilters: false,
  ...
  methods: {
    ...
    fetchData() {
      this.loading = true;
      Promise.all([
        this.fetchLogs(['id', 'username','ip', 'userAgent', 'toUser', 'type', 'value','clientIds','resellerIds','clientIdsIndex','resellerIdsIndex','logType', 'createdAt'], true, ['ip'],this.orderByFilter),
        this.fetchLogs(['id', 'username','ip', 'userAgent', 'toUser', 'type', 'value','clientIds','resellerIds','clientIdsIndex','resellerIdsIndex','logType', 'createdAt'], true, ['username'],this.orderByFilter),
        this.fetchLogs(['id', 'username','ip', 'userAgent', 'toUser', 'type', 'value','clientIds','resellerIds','clientIdsIndex','resellerIdsIndex','logType', 'createdAt'], true, ['type'],this.orderByFilter),
    ...
    async fetchLogs(groupByFilter, paginate, selectAttributes, orderByFilter) {
      ...
      try {
        let response = await apiService.axiosToBackend().get("/api/log/system", {
            params: {
