...
<!--      <template v-slot:expanded-row="{ headers, item }">
        <tr style="border-color: #63adf2;border-style: solid;" class="pr-0 pl-1">
...
export default {
  ...
  methods: {
    ...
    async fetchLogs(groupByFilter, paginate, selectAttributes, orderByFilter) {
      try {
        let response = await apiService.axiosToBackend().get("/api/log/access", {
            params:{
