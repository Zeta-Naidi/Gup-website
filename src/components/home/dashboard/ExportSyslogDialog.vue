...
export default {
  ...
  methods: {
    ...
    async exportSyslog() {
      ...
      try {
        const response = await apiService.axiosToBackend().get("/api/event/export", {
          params: {
            filters: JSON.stringify({
              ...
              modExport: this.modExportTableEvents === 1 ? 'SYSLOG_RAW' : 'SYSLOG_JSON'
            }),
          },
        });
        ...
        if (this.modExportTableEvents === 1) {
          //RAW CASE
          const blob = new Blob([response.data.payload], { type: 'text/plain' });
          const link = document.createElement('a');
          link.href = window.URL.createObjectURL(blob);
          link.download = `${this.fileName}.log`;
          document.body.appendChild(link);
          link.click();
          document.body.removeChild(link);
        } else {
          // PDF CASE
          await exportService.exportSyslogEventToPdf({
    ...
    async exportSyslog2() {
      try {
        ...
        // RAW CASE
        if (this.modExportTableEvents === 1) {
          const blob = new Blob([response.data.payload])
          const link = document.createElement('a')
          link.href = window.URL.createObjectURL(blob)
          link.download = this.fileName + '.log';
          document.body.appendChild(link);
        ...
        // PDF CASE
          ...
          await exportService.exportSyslogEventToPdf(options);
        }
        this.waitingExportToFinish = false;
      } catch (e) {
        this.waitingExportToFinish = false;
